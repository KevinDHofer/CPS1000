...
gene_ranks <- setNames(ranked_genes_unique$stat, ranked_genes_unique$Description)

gene_ranks_knisbacher <- gene_ranks
#write.csv(gene_ranks_knisbacher, "~/Dropbox/Medizin/Forschung/Aktuelle Forschungsprojekte/CLL CPS1000/analysis/gene_ranks_knisbacher.csv")
...
#saveRDS(fgsea_hallmark_df_knis, "fgsea_hallmark_df_knis.rds")

topPathways_hallmark_clean <- fgsea_hallmark_df_knis$pathway[1:10]

#ggplot2
plot_fgsea_h <- fgsea_hallmark_df_knis |> filter(pathway %in% topPathways_hallmark_clean, padj <0.1) |> 
...
KEGG_gene_sets <- msigdbr(species = "Homo sapiens", collection = "C2", subcollection = "CP:KEGG_LEGACY") %>%
  dplyr::select(gs_name, gene_symbol) %>%
  split(x = .$gene_symbol, f = .$gs_name)
...
#saveRDS(fgsea_kegg_df_knis, "fgsea_kegg_df_knis.rds")

topPathways_kegg_clean_knis <- fgsea_kegg_df_knis$pathway[1:10]

# plot top10 for GO
plot_fgsea_kegg <- fgsea_kegg_df_knis |> filter(pathway %in% topPathways_kegg_clean_knis) |> 
