...
plotTab <- pTab %>% filter(gene != "IGHV.status") %>%
  ...
  mutate(gene = ifelse(p.adj < 0.1, gene, "Below 10% FDR"),
         drug = factor(drug, levels = drugOrder2),
         gene = factor(gene, levels = names(colorList))) 

Fig5a <- ggplot(plotTab, aes(x=drug, y = -log10(p), col = gene, shape = gene)) +
  geom_point(size =2, alpha = 0.8) + scale_color_manual(values = colorList) +
  scale_shape_manual(values = shapeList) + theme_bw() +
  labs(x="", color = "Genetic aberration", shape = "Genetic aberration")+
