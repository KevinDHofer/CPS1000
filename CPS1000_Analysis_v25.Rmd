...
FigS11 <- plot_grid(r1, NULL, nrow = 2, rel_heights = c(0.2,0.8)) & theme(plot.margin=margin(r=1,l=1,t=1,b=1,unit="cm"))
...
cll_corr_df <- scale(cll_df[3:ncol(cll_df)])
...
col_anno <- HeatmapAnnotation(
  ...
  "All data types" = anno_barplot(
    ...
    bar_width  = 0.7,
    ylim       = c(0, max(r2_mat, na.rm = TRUE)),
    height     = unit(1.5, "cm"),
...
c2 <- plot_grid(FigS7c, FigS7f, NULL,
                  align = "v", axis = c("l", "r"), ncol = 1, rel_heights= c(0.6, 0.275, 0.125), 
...
FigS8e <- nlrp3_plot + theme(plot.margin = margin(t = 1, r = 5, b = 1, l = 1, unit = "cm"))
```
...
FigS8f <- ddx3y_plot + theme(plot.margin = margin(t = 1, r = 5, b = 1, l = 1, unit = "cm"))
```
...
FigS8c <- ggdraw() + draw_image(figs8c, scale = 1.5)
FigS8d <- ggdraw() + draw_image(figs8d, scale = 1.5)

r2c1 <- plot_grid(FigS8e, FigS8f, nrow = 2, rel_heights = c(0.5, 0.5), 
                  labels=c("E", "F"), label_size=14)

r2c2 <- plot_grid(r2c1, NULL, ncol =2, rel_widths = c(0.65, 0.35))

r2 <- plot_grid(FigS8c, r2c2,
...
r3 <- plot_grid(FigS8d, NULL,
                  ncol = 2, rel_widths = c(0.7, 0.3), 
                  labels=c("D", ""), label_size=14)

FigS8 <- plot_grid(r1, r2, r3, NULL, nrow =4, rel_heights = c(0.15, 0.3, 0.3, 0.2)) & theme(plot.margin=margin(l=1,t=1,b=1,unit="cm"))
...
Fig7 <- plot_grid(row1, row2, row3, NULL, nrow=4, rel_heights = c(0.2, 0.2, 0.175, 0.425)) & theme(plot.margin=margin(r=1,l=1,t=1,b=1,unit="cm"))
...
mcll_ibr_ttt <- screen_means_ighv |> 
  ...
  mutate(ibr_mean_viab = ifelse(mean_viab > ttt_value, "high", "low"))
...
flu_level3 <- flu_merged_pre |> 
  ...
  theme_figures+
  theme(legend.position = "none")+
  labs(title = bquote(bold("Fludarabine response (0.4 "*mu*"M)")), y="Viability", x= "Next treatment\nduring follow-up")
...
```{r}
tx <- read_excel((paste0(filepath,"data/supp_tables.xlsx")), sheet = "S4_agents_treatment_CLL")
...
```{r}
long <- read_excel((paste0(filepath,"data/supp_tables.xlsx")), sheet = "S5_regimes_longitudinal_CLL")
