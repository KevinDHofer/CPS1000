...
age <- merge(dob, survT, by.x="Patient ID", by.y="patID") |> 
  ...
  mutate(age_sampling = as.numeric(difftime(as.POSIXct(sampleDate), as.POSIXct(dob), units = "days"))/365) |> 
...
viabTab.mean <- merged_table_long |> 
  ...
  filter(diagnosis == "CLL") |> 
  merge(age, by = c("patientID", "sampleID"))
