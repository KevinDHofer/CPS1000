...
p1 <- CLL_df |> 
    ggplot(aes(x=viab_mean, y=ORR, color=Category)) +
...
p1 <- p1+ geom_line(data = pred_data, 
                 aes(x = viab_mean, y = ORR), color = "grey80", inherit.aes = FALSE)+
          geom_line(data = pred_data, 
                 aes(x = viab_mean, y = ORR+20), color = "grey90", linetype="dashed", inherit.aes = FALSE)+
          geom_line(data = pred_data, 
                 aes(x = viab_mean, y = ORR-20), color = "grey90", linetype="dashed", inherit.aes = FALSE)
...
#ORR, CLL
parameters <- c("HS_mean", "Einf_mean", "pIC50_mean", "R2_mean", "AUC_mean", "viab_mean")
cor_data <- c()
...
filter(str_detect(parameter, "mean")) |> 
  ...
  ggplot(aes(y=parameter, x=abs(cor), fill=outcome))+
  geom_col(position=position_dodge())+
  facet_wrap(~diagnosis, ncol=2)+
  labs(title="Correlation with clinical response", y="Parameter (mean)", x="Absolute Pearson correlation coefficient",
  fill = "Direction")+
  theme_bw()+
  ...
  legend.title = element_text(size=8, face="bold"), legend.text=element_text(size=8), axis.title = element_text(size=8), plot.title = element_text(hjust=0.5, size=8, face="bold"))+
  xlim(0,1)+
  scale_y_discrete(labels=c("AUC", "Einf", "Hill coefficient", "pIC50", "R2", "Mean viability"))
Fig7d