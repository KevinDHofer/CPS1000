...
conc_diff <- conc_range |> 
  ...
  labs(x = "", y = expression(log[10]~" Concentration ("~mu*"M)"))+
  scale_y_log10()+
  scale_x_discrete(labels = c("CPS1000", "Kornauth\net al. 2022", "Tyner\net al. 2018"))+
  scale_fill_manual(values = c("#A6CEE3", "#FB9A99", "#CAB2D6"))

Fig2d <- conc_diff & theme(plot.margin = margin(r=1, l=1, t=1, b=0.5, unit="cm"))
...
```{r, echo = FALSE}
#Fig. 2 multipanel
line1 <- plot_grid(Fig2a,Fig2c, ncol=2, align = "h", rel_widths = c(0.6, 0.4), labels=c("A", "D"), label_size=14)
line2 <- plot_grid(Fig2d, Fig2b, ncol=2, align = "v", axis = "tb", rel_widths = c(0.25, 0.75), labels=c("B", "C"), label_size=14)
...
#Fig. 3 multipanel
line1 <- plot_grid(Fig3c, Fig3a , ncol=2, rel_widths = c(0.65, 0.35), labels=c("A", "B"), label_size=14)
...
```
# Comparing variance explained by regression models
...
inclSet<-list(RNA=rnaPCA, meth=methPCA, gen=genData, IGHV=IGHVData, 
              Mcluster = Mcluster, drugs=viabMat)
  ...
  for (eachMeasure in names(glmResult)) {
    ...
    g <- ggplot(plotTab,aes(x=variable, y=mean, fill= variable)) + 
      ...
      theme_classic() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size =12), 
                              ...
                              legend.position = "none") + 
      scale_fill_brewer("Set1",type = "qual") + coord_cartesian(ylim = c(0,0.7)) +
      ylab("R2") + xlab("") + ggtitle(plotTitle)
...
**Plot some examples, based on the drugs show in Fig5A in the JCI paper.**
```{r, fig.height=10, fig.width=10}
showDrugs <- c("Fludarabine","Nutlin-3a", "Doxorubicine","Ibrutinib",
               "Idelalisib", "Trametinib", "Rapamycin", "PRT062607")
showVars <- varList[showDrugs]
gridExtra::grid.arrange(grobs = showVars, ncol =4)
```

#plot heatmap
```{r}
#extract mean R2 across 20 models
omics_types <- c("genetic(24)", "expression(20)", "methylation(20)", "IGHV(1)", "methCluster(1)", "all(66)")
...
#separate the "all(66)" column for the annotation
omics_only <- r2_mat[, colnames(r2_mat) != "all(66)"]
colnames(omics_only) <- c("Genetic (24)", "Gene expression (20)", "Methylation (20)", "IGHV (1)", "Methylation cluster (1)")
all_vals    <- r2_mat[, "all(66)"]
...
h_var <- Heatmap(t(omics_only), name = "R2", col = color_fun,
                 ...
                 show_column_dend = TRUE, show_parent_dend_line = TRUE,
...
Fig4c <- grid.grabExpr({
  ...
  decorate_annotation("All data types", {
    grid.lines(
      x = unit(c(0, 1), "npc"),
...
```{r, echo = FALSE}
#multipanel
row3 <- plot_grid(Fig4c, NULL, ncol = 2, rel_widths = c(0.85, 0.15))

Fig4 <- plot_grid(Fig4a,Fig4b, row3, NULL, nrow = 4, rel_heights = c(0.4, 0.125,0.2,0.275), 
                  labels=c("A", "B", "C", ""), label_size=14) & theme(plot.margin=margin(r=1,l=1,t=1,b=1,unit="cm"))
...
#select U-CLL patients with RNAseq data: 79 patients, 4 patients with DDX3X
...
#AML: filter dataset for non-FLT3e inhibitors and fit linear model for ORR
...
#plot for AML with corridor: mean viability vs ORR
p3 <- AML_outcomes |> 
    ggplot(aes(x=viab_mean, y=orr)) +
...
#AML: filter dataset for non-FLT3e inhibitors and fit linear model for CRR
lm_model_aml_crr <- lm(crr ~ viab_mean, data = filtered_data_aml)
pred_data_aml$crr <- predict(lm_model_aml_crr, newdata = pred_data_aml)


#plot for AML with corridor: mean viability vs CRR
p4 <- AML_outcomes |> 
    ggplot(aes(x=viab_mean, y=crr)) +
...
#correlation of ORR with the Hill slope in CLL
hill_model <- lm(orr ~ HS_mean, data = CLL_outcomes)

pred_hill <- data.frame(HS_mean = seq(
  min(CLL_outcomes$HS_mean, na.rm = TRUE),
  max(CLL_outcomes$HS_mean, na.rm = TRUE),
  length.out = 100))

pred_hill$orr <- predict(hill_model, newdata = pred_hill)
pred_hill$se <- predict(hill_model, newdata = pred_hill, se.fit = TRUE)$se.fit
pred_hill$lower <- pred_hill$orr - 1.96 * pred_hill$se
pred_hill$upper <- pred_hill$orr + 1.96 * pred_hill$se

#plot
hill <- CLL_outcomes |> 
  ggplot(aes(x=HS_mean, y=orr)) +
  geom_line(data=pred_hill, aes(x=HS_mean, y=orr), color="grey80")+
  geom_line(data=pred_hill, aes(x=HS_mean, y=upper), color="grey80", linetype="dashed")+
  geom_line(data=pred_hill, aes(x=HS_mean, y=lower), color="grey80", linetype="dashed")+
  geom_point(aes(size=nr_pat, color=Category), alpha=0.8) +
  geom_text_repel(aes(label=Drug), size=3, color="black", 
                  box.padding = 0.5,
                  point.padding = 0.75,
  ...
  labs(title = "CLL", subtitle = "ORR ~ Hill coefficient", x = "Mean Hill coefficient", y = "ORR", color="Drug type", size="Nr. of patients\nin clinical trial")+
  scale_color_manual(values=colors_drug_type, labels=labels_drug_type)+
  ylim(0,100)

FigS10d <- hill + theme(plot.margin = margin(l = 1, r = 1, t=1, b=1, unit = "cm"))
...
smart <- data_all |> 
  filter(Drug %in% smart_drugs, treatment_spec %in% c("Ibrutinib", "Venetoclax", "Venetoclax-Rituximab", "Venetoclax + Obinutuzumab", "Idelalisib", "Rituximab-Idelalisib", "DA-Induction", "Palbociclib"),
...
FigS10c <- smart_orr + plot_annotation("CLL and AML (Liebers et al. 2023)", theme = theme(
  plot.margin = margin(t=1, b=1, l=1, unit = "cm"),
  plot.title = element_text(hjust=0.05, size=8, face="bold")))
FigS10c
...
#plot
response_curve <- screen_long_ven_cll |> 
...
FigS10a <- response_curve+theme(plot.margin = margin(t=1, b=3, r=1, l=1, unit = "cm"))
...
Fig7d <- Fig7d + theme(plot.margin = margin(t=1.5, b=1, l=1, unit = "cm"))
Fig7d
...
pairwise_df_os <- combn(unique(km_surv_os$group), 2, simplify = FALSE) %>%
  map_dfr(~{
    ...
    d <- km_surv_ttt %>% filter(group %in% c(g1, g2))
    sd <- survdiff(Surv(OS) ~ group, data = d)
...
#combine
Fig7f <- (Fig7d1+Fig7d2) + plot_layout(guides = 'collect') & 
...
col2row1 <- plot_grid(Fig7d, Fig7e, NULL,
                  align = "v", axis = c("l", "r"), ncol = 1, rel_heights = c(0.4, 0.4, 0.2), 
...
## Check RNAseq signatures of M-CLL with shorter TTT
```{r}

#divide M-CLL into low and high ibrutinib group based on cutoff for TTT
mcll_ibr_ttt <- screen_means_ighv |> 
  filter(diagnosis == "M-CLL", drug == "Ibrutinib") |> 
  mutate(ibr_mean = ifelse(mean_viab > ttt_value, "high", "low"))
...
resLFC <- lfcShrink(dds, coef="condition_mcll_high_vs_low", type="apeglm")
resLFC
...
FigS9n <- fig_mcl_hallmark + theme(plot.margin = margin(t=2, l=1, b=1, r=1, unit = "cm"))
FigS9n

```


## Modeling clinical outcomes by drug response in CLL
```{r}
...
## Modeling clinical outcome by drug response in AML
```{r}
