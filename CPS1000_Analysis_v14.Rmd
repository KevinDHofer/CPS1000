...
#correlation of ORR with variability of drug response in CLL
CLL_outcomes <- CLL_outcomes |> 
    mutate(cov = viab_sd/viab_mean)

cov_model <- lm(orr ~ cov, data = CLL_outcomes)

pred_cov <- data.frame(cov = seq(
  min(CLL_outcomes$cov, na.rm = TRUE),
  max(CLL_outcomes$cov, na.rm = TRUE),
  length.out = 100))

pred_cov$orr <- predict(cov_model, newdata = pred_cov)
pred_cov$se <- predict(cov_model, newdata = pred_cov, se.fit = TRUE)$se.fit
pred_cov$lower <- pred_cov$orr - 1.96 * pred_cov$se
pred_cov$upper <- pred_cov$orr + 1.96 * pred_cov$se

#plot
sd_cll <- CLL_outcomes |> 
  ggplot(aes(x=cov, y=orr)) +
  geom_ribbon(data=pred_cov, 
                aes(x=cov, ymin=upper, ymax=100), 
                inherit.aes = FALSE, 
                fill="tomato1", 
                alpha=0.1)+
  geom_ribbon(data=pred_cov, 
                aes(x=cov, ymin=0, ymax=lower), 
                inherit.aes = FALSE, 
                fill="steelblue", 
                alpha=0.1) +
  geom_line(data=pred_cov, aes(x=cov, y=orr), color="grey80")+
  geom_line(data=pred_cov, aes(x=cov, y=upper), color="grey80", linetype="dashed")+
  geom_line(data=pred_cov, aes(x=cov, y=lower), color="grey80", linetype="dashed")+
  geom_point(aes(size=nr_pat, color=Category), alpha=0.8) +
  geom_text_repel(aes(label=Drug), size=3, color="black", 
                  box.padding = 0.5,
                  point.padding = 0.75,
  ...
  labs(title = "CLL", subtitle = "ORR ~ CoV", x = "Coefficient of variation (viability)", y = "ORR", color="Drug type", size="Nr. of patients\nin clinical trial")+
  scale_color_manual(values=colors_drug_type, labels=labels_drug_type)+
  ylim(0,100)

FigS10b <- sd_cll + theme(plot.margin = margin(l = 1, t=1, b=1, unit = "cm"))
FigS10b
