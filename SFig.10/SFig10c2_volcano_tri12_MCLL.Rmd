...
df_2 <- means_merged |> 
  group_by(drug) |> 
    summarize(p_val = mean(neg_log10_p)) |> 
    ungroup()
...
dat <- subset(df_3, diff_viab > -40 & diff_viab < 20)
dat$drug <- df_3$drug
dat <-  dat |> 
...
#calculate difference in viability instead of log2fc
df_wide_tri <- pivot_wider(df_tri, names_from = "trisomy12", values_from = "viab") 
...
plot5 <- ggplot(df_4m, aes(x = diff_viab, y = p_val, label=drug)) +
  ...
  labs(
    ...
    annotate("text", label="more active\nin trisomy12", y=10, x=-5, size=3)+
    annotate("text", label="more active\nin wt", y=10, x=5, size=3)
plot5          


SFig10c2 <- plot5 + theme(plot.margin = margin(t=1, l=1, r=1, unit = "cm"))
SFig10c2