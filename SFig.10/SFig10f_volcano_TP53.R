...
#filter for samples with TP53 annnotation
means_merged_mut <- means_merged |> 
  filter(!is.na(TP53))

#create new table with genetic information: mean-mut and mean-nonmut
df_tp53 <- means_merged_mut |> 
  group_by(TP53, drug) |> 
...
df_wide_tp53 <- pivot_wider(df_tp53, names_from = "TP53", values_from = "mean(viab)") |> 
  dplyr::rename(tp53 = "1", none = "0")

df_1 <- df_wide_tp53 |> 
  mutate(diff_viab = (tp53-none)/(tp53)*100)
...
p_value = sapply(means_merged$drug, function(d) {
  tp53_yes <- means_merged %>% 
    filter(TP53 == "1" & drug == d) %>% 
    pull("mean(viability)")
  tp53_no <- means_merged %>% 
    filter(TP53 == "0" & drug == d) %>% 
    pull("mean(viability)")
  wilcox.test(tp53_yes, tp53_no)$p.value
})
...
dat <- subset(df_3, diff_viab > -20 & diff_viab < 20)
dat$drug <- df_3$drug
dat <-  dat |> 
...
#calculate difference in viability instead of log2fc
df_wide_tp53 <- pivot_wider(df_tp53_2, names_from = "TP53", values_from = "mean(viab)") 
...
p_values <- merged %>%
  ...
  summarise(
    ...
    mean_yes = mean(viability[TP53 == "1"], na.rm = TRUE),
    mean_no = mean(viability[TP53 == "0"], na.rm = TRUE)
  ) %>%
  ungroup() %>%
...
df_4m$Pathway_mod2 <-  factor(df_4m$Pathway_mod2, levels = c("ABL (BCR)", "Apoptosis", "B-cell receptor", "Bromodomain/PLK", "Cell cycle control", "Chemotherapy", "DNA damage response", "Histone methylation", "JAK/STAT", "MAPK", "PI3K/AKT/mTOR", "Stress pathway","other"))  

#use colors from heatmap Fig3
annotation_colors_row <- setNames(c("lightgrey", "#FCCDE5", "#B3DE69", "#FDB462", 
                                    "#80B1D3", "#FB8072", "#BEBADA", "#FFFFB3", 
                                    "#8DD3C7", "#A6CEE3", "#6DBD57", "#DE9E83", 
                                    "#BC80BD"), 
                                  c("other", "DNA damage response", "Chemotherapy" ,  
                                    "MAPK" , "B-cell receptor", "PI3K", "Stress pathway", 
                                    "Bromodomain/PLK", "JAK/STAT", "Apoptosis", 
...
plot5 <- ggplot(df_4m, aes(x = diff_viab, y = p_val, label=drug)) +
  ...
  annotate("text", label="more active\nin mutated", y=15, x=-10, size=3)+
  annotate("text", label="more active\nin wt", y=15, x=10, size=3)
plot5          

SFig10f <- plot5 + theme(plot.margin = margin(t=1, l=1, r=5.5, unit = "cm"))
SFig10f
