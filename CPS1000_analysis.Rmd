...
#load data from Knisbacher et al. 2022
...
#prepare data with clinical outcomes for CLL and AML (remove drugs which were tested in subgroups)
...
AML_outcomes <- AML_outcomes |> 
  filter(orr != "NA", !(drug %in% c("Imatinib", "Menin-MLL Inhibitor")))  |> 
  mutate(nr_pat = as.numeric(nr_pat), orr = as.numeric(orr), crr = as.numeric(crr)) |> 
  merge(drugs[,c("Drug", "Category")], by.x="drug", by.y = "Drug")
...
## Assess differential RNA signatures in M-CLL with shorter TTT
```{r}

#divide M-CLL into low and high ibrutinib mean viability based on cutoff for TTT
mcll_ibr_ttt <- screen_means_ighv |> 
  filter(diagnosis == "M-CLL", drug == "Ibrutinib") |> 
  mutate(ibr_mean_viab = ifelse(mean_viab > median(mean_viab), "high", "low"))

median(mcll_ibr_ttt$mean_viab)

mcll_ibr_ttt |> 
  group_by(ibr_mean_viab) |> 
  summarize(n=n())

#get RNAseq data
mcll_ibr_expr <- count_df_cps_clean |> 
  dplyr::select(ensembl_gene_id, external_gene_name, any_of(mcll_ibr_ttt$patientID))

#select M-CLL patients with low ibr viability
mcll_low_list <- mcll_ibr_ttt |> 
  filter(ibr_mean_viab == "low") |> 
  pull(patientID)

#prepare matrix for DESeq2
countData_mcll <- as.matrix(mcll_ibr_expr[, -c(1, 2)])
rownames(countData_mcll) <- mcll_ibr_expr$ensembl_gene_id

paste0("M-CLL samples with RNAseq and LOW ibrutinib response: ", length(mcll_low_list))
paste0("M-CLL samples with RNAseq and HIGH ibrutinib response: ", length(colnames(countData_mcll))-length(mcll_low_list))

condition_mcll <- factor(ifelse(colnames(countData_mcll) %in% mcll_low_list, "low", "high"))

dds <- DESeqDataSetFromMatrix(countData = countData_mcll, 
                              colData = DataFrame(condition_mcll), 
                              design= ~ condition_mcll)

dds$condition_mcll <- relevel(dds$condition_mcll, ref = "low")
...
#drop genes with padj = NA
#positive log2FoldChange indicates that the gene is up-regulated in ibr high and vice versa
DEres <- results(dds, tidy=TRUE)
DEres_mcll <- DEres |> 
  merge(mcll_ibr_expr[,c("ensembl_gene_id", "external_gene_name")], by.x = "row", by.y = "ensembl_gene_id") |> 
  drop_na(padj, row, stat) |> 
  filter(external_gene_name != "") |> 
  arrange(abs(stat))

#remove duplicates by keeping the entry with the highest absolute stat value
DEres_mcll <- DEres_mcll %>%
  group_by(external_gene_name) %>%
  slice_max(abs(stat), n = 1) %>%
  ungroup()

#create a named vector
gene_ranks_mcll <- setNames(DEres_mcll$stat, DEres_mcll$external_gene_name)

#run fgsea
fgsea_hallmark_mcll <- fgsea(
  pathways = hallmark_gene_sets,
  stats = gene_ranks_mcll,
  minSize = 15,
  maxSize = 500
)
...
hallmark_plot_mcll <- fgsea_hallmark_mcll |> 
  arrange(pval) |> 
  slice_head(n=10) |> 
  mutate(direction = as.character(ifelse(sign(NES) <0, "up-regulated in\nstrong responders", "up-regulated in\npoor responders")),
         is_top10 = pathway %in% top10_common)
...
FigS11c <- fig_mcl_hallmark + theme(plot.margin = margin(t=1, l=1, b=1, r=1, unit = "cm"))
FigS11c
