...
```{r}
#diagnosis
colors_diag <- setNames(c("#1F78B4", "#B2DF8A", "grey", "#A6CEE3", "#E31A1C", "#FB9A99", "#FF7F00", "#CAB2D6"), c("MCL", "CLL", "T-PLL", "other", "AML", "T-ALL", "B-ALL", "B-PLL"))

colors_diag_IGHV <- setNames(c("#1F78B4", "#B2DF8A", "#33A02C", "#A6CEE3", "#E31A1C", "#FB9A99", "#FF7F00", "#CAB2D6", "grey"), c("MCL", "U-CLL", "M-CLL", "other", "AML", "T-ALL", "B-ALL", "B-PLL", "T-PLL"))
...
colors_del17p <- setNames(c("#FC8D62", "grey60"), c(1, 0))

colors_tri12 <- setNames(c("#FFD92F", "grey80"), c(1, 0))
...
FigS12 <- plot_grid(r1, NULL, nrow = 2, rel_heights = c(0.2,0.8)) & theme(plot.margin=margin(r=1,l=1,t=1,b=1,unit="cm"))
...
conc_diff <- conc_range |> 
  ...
  labs(x = "", y = expression(paste("Concentration (", mu, "M)")))+
...
#heatmap
h1 <- Heatmap(z, name = "Z-score", show_row_names = TRUE, show_column_names=FALSE, row_names_gp = gpar(fontsize = 8), width = ncol(z)*unit(0.25, "mm"), height = nrow(z)*unit(3.5, "mm"), col = color_fun_reversed, top_annotation = ha, left_annotation = ha2, show_column_dend = FALSE, row_km = 3, row_title = c("1", "2", "3"), row_title_gp = gpar(fontsize = 8), 
...
show_row_names = FALSE, show_column_names=FALSE, width = ncol(w)*unit(3.1, "mm"), height = nrow(w)*unit(3.1, "mm"), show_row_dend=TRUE, row_dend_side = "left", show_column_dend=FALSE,
row_dend_width = unit(7, "mm"), heatmap_legend_param = list(title_gp = gpar(fontsize = 8, fontface = "bold"), labels_gp = gpar(fontsize = 8)))

Fig3a_top <- draw(h2, heatmap_legend_side = "right", padding = unit(c(2, 2, 0, 2), "mm"))
...
Fig3a <- plot_grid(grid.grabExpr(draw(h2, heatmap_legend_side = "right")), Fig3a_bottom_aligned, ncol=1, rel_heights = c(0.5, 0.5))
...
FigS4c <- aml_cyt + aml_ven + plot_annotation(title = "AML", theme = theme(plot.title = element_text(hjust=0.5, size=8, face="bold"))) + theme(plot.margin = margin(t=1, unit = "cm"))
FigS4c
```
...
figb <- plot_grid(NULL, Fig3c, NULL, ncol=1, rel_heights = c(0.05, 0.925, 0.025))
...
FigS6 <- plot_grid(alldrugslasso, NULL, nrow=2, rel_heights = c(0.25, 0.75)) & theme(plot.margin = margin(t=1,b=1, r=5,l=1, unit = "cm"))
...
col_anno <- HeatmapAnnotation(
  ...
  annotation_name_gp   = gpar(fontsize = 8),
  annotation_legend_param = list(
    Pathway = list(title = "Pathway", title_gp = gpar(fontsize = 8, fontface = "bold"),
      labels_gp = gpar(fontsize = 8), ncol = 2)
  )
)
...
h_var <- Heatmap(t(omics_only), name = "R2", col = color_fun,
                 ...
                 width = ncol(omics_only)*unit(40, "mm"), height = ncol(omics_only)*unit(5, "mm"), 
                 show_column_dend = TRUE, show_parent_dend_line = FALSE,
                 column_km = 6, column_title = NULL,
                 top_annotation = col_anno, heatmap_legend_param = list(title  = expression(bold(R^2)), title_gp = gpar(fontsize = 8, fontface = "bold"), labels_gp = gpar(fontsize = 8)))
...
#multipanel
#Fig4
row3 <- plot_grid(Fig4c, NULL, ncol = 2, rel_widths = c(0.95, 0.05))
...
FigS7 <- plot_grid(top_rows, NULL, nrow=2, rel_heights= c(0.95, 0.05), 
                   labels=c("", ""), label_size=14) & theme(plot.margin = margin(t=1, b=1, l=1, r=1, unit = "cm"))
...
ddx3y_id <- DEres_cps |> 
  filter(external_gene_name == "DDX3Y") |> 
  pull(row)

ddx3y <- normalized_counts[rownames(normalized_counts) == ddx3y_id]
ddx3y <- as.data.frame(ddx3y) 
ddx3y$patientID <- colnames(normalized_counts)
ddx3y <- ddx3y |> 
    mutate(DDX3X = ifelse(patientID %in% ddx3x, 1, 0)) |> 
    left_join(metadata[,c("Patient.ID", "gender")], by=c("patientID"="Patient.ID"))

#plot
ddx3y_plot <- ddx3y |>
  mutate(gender = case_when(gender == "f" ~ "Female",
                              TRUE ~ "Male"),
         DDX3X = factor(DDX3X, levels = c(0, 1))) |>
...
FigS8f <- ddx3y_plot + theme(plot.margin = margin(t = 1, r = 2, b = 1, l = 1, unit = "cm"))
FigS8f
```
...
r3 <- plot_grid(FigS8e, FigS8f, NULL,
                  ncol = 3, rel_widths = c(0.25, 0.25, 0.5), 
                  labels=c("E", "F", ""), label_size=14)

FigS8 <- plot_grid(r1, r2, r3, NULL, nrow =4, rel_heights = c(0.15, 0.25, 0.15, 0.45)) & theme(plot.margin=margin(l=1,t=1,b=1,unit="cm"))
...
row1<- plot_grid(FigS9a, FigS9b, NULL, ncol = 3, rel_widths = c(0.2, 0.25, 0.55), labels = c("A", "B", ""), label_size = 14)
...
smart_examples <- smart |> 
  ...
  geom_jitter(alpha = 0.5, size=1, alpha = 0.8, width=0.05)+
  facet_wrap(~Drug) +
  theme_figures_facet_angle45_x_legend +
...
## Kaplan-Meier plots (based on ibrutinib response)
```{r, fig.width=8, fig.height=4}

#maxstat for OS and TTT
km_surv <- surv_combined |> 
  filter(!is.na(IGHV.status), !is.na(OS), drug == "Ibrutinib")
...
paste0("OS cutpoint = ", round(res.cut.os$cutpoint$cutpoint,2))
...
paste0("TTT cutpoint = ", round(res.cut.ttt$cutpoint$cutpoint,2))
...
#plot TTT
ttt <- survfit2(Surv(TTT, treatment) ~ ibr_resistance_ttt+IGHV.status, data = km_surv)
...
#combine
Fig7d <- (km_os+km_ttt) + plot_layout(guides = 'collect') & 
  theme(legend.position='right', plot.margin = margin(t=0.5, l=0.5, r=0.5, unit = "cm")) &
...
row3 <- plot_grid(Fig7c, Fig7d, ncol = 2, align = "h", axis = c("tb"), rel_widths = c(0.3, 0.7), 
                            labels=c("C", "D"), label_size=14)
...
Fig7 <- plot_grid(row1, row2, row3, NULL, nrow=4, rel_heights = c(0.25, 0.25, 0.175, 0.325)) & theme(plot.margin=margin(r=1,l=1,t=1,b=1,unit="cm"))
...
flu_response <- all_predictions_flu |> 
  ...
  geom_line()+
  theme_figures+
  labs(title = "Fludarabine", y="Viability", x=bquote("Concentration ("*mu*"M)"), color = "Retreatment\nduring follow-up")+
...
FigS10 <- plot_grid(row1, row2, row3, NULL, nrow=4, rel_heights = c(0.2, 0.2, 0.2, 0.4)) & theme(plot.margin=margin(r=1,l=1,t=1,b=1,unit="cm"))
...
```{r}
aml <- read_excel((paste0(filepath,"data/supp_tables.xlsx")), sheet = "S6_trials_AML")
...
```{r}
cll <- read_excel((paste0(filepath,"data/supp_tables.xlsx")), sheet = "S7_trials_CLL") |> 
  mutate(across(everything(), as.character))
