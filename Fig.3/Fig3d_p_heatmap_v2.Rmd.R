...
drugs <- drugs |> dplyr::select(drug, Pathway_mod)

plotTab2 <- left_join(plotTab, drugs, by="drug")

#plot
library(ggtext)

main_plot <- ggplot(plotTab2, aes(x=diagCompare, y = drug, fill = pSign)) + 
...
anno_plot <- ggplot(plotTab2, aes(x = diagCompare, y = drug, fill = Pathway_mod)) +
  geom_tile() +
...
Fig3d <- ggdraw() + 
  ...
  draw_plot(anno_plot,x=0.855,y=0.067,width=0.0325,height=0.907)
Fig3d